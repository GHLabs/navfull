(function() {
  var Autocompleter, Bag42Completer, CategoryPrediction, ContactCompleter, FavouritesCompleter, GeocoderCompleter, GoogleCompleter, GoogleLocation, HistoryCompleter, Location, LocationHistory, LocationPrediction, OSMCompleter, POICategoryCompleter, Prediction, RemoteAutocompleter, accent_insensitive_pattern, completers, generate_area_completers, get_all_predictions, google_url, hel_geocoder_address_url, letters_to_accents, navigate_to_location, navigate_to_poi, nominatim_url, pred_list, render_autocomplete_results, supported_completers, test_completer, _ref,
  ...
  Location = (function() {
    function Location(name, coords, tag) {
      this.name = name;
      this.coords = coords;
      this.tag = tag;
    }
  ...
  LocationHistory = (function() {
    ...
    LocationHistory.prototype.remove = function(loc) {
      this.array.splice(this.array.indexOf(loc.to_json()), 1);
      this.history.splice(this.history.indexOf(loc), 1);
      return localStorage[this.ls_id] = JSON.stringify(this.array);
    };
  ...
  Prediction = (function() {
    ...
    Prediction.prototype.render = function() {
      var $add_el, $el, $li_el, dest_page, icon_html, name, tag, _ref1;
      icon_html = '';
      name = this.name;
      tag = this.tag;
      ...
      if (tag != null) {
        this.location.icon = 'static/images/star.svg';
      }
  ...
  FavouritesCompleter = (function(_super) {
    __extends(FavouritesCompleter, _super);

    function FavouritesCompleter() {
      return FavouritesCompleter.__super__.constructor.apply(this, arguments);
    }

    FavouritesCompleter.DESCRIPTION = "Favourites";

    FavouritesCompleter.prototype.get_predictions = function(query, callback, args) {
      var location, pred_list, _i, _ref1;
      pred_list = [];
      _ref1 = location_favourites.history;
      for (_i = _ref1.length - 1; _i >= 0; _i += -1) {
        location = _ref1[_i];
        if (query.length && location.tag.toLowerCase().indexOf(query.toLowerCase()) !== 0) {
          continue;
        }
        pred_list.push(new LocationPrediction(location));
    ...
    return FavouritesCompleter;

  })(Autocompleter);

  ContactCompleter = (function(_super) {
    __extends(ContactCompleter, _super);

    function ContactCompleter() {
      this.onErrorContacts = __bind(this.onErrorContacts, this);
      this.onSuccessContacts = __bind(this.onSuccessContacts, this);
      return ContactCompleter.__super__.constructor.apply(this, arguments);
    }
  ...
  supported_completers = {
    ...
    favourites: new FavouritesCompleter
  };
  ...
  render_autocomplete_results = function(args, new_preds, error, completer) {
    var $add_el, $el, $input, $li_el, $ul, key, pred, seen, seen_addresses, seen_streets, tmp, tmp_render, _i, _len, _ref1, _ref2;
    $ul = args.$ul;
    $input = args.$input;
    if (completer == null) {
    ...
    for (_i = 0, _len = pred_list.length; _i < _len; _i++) {
      ...
      seen[key] = true;
      tmp_render = pred.render();
      $el = tmp_render["el"];
      $li_el = tmp_render["li_el"];
      $add_el = tmp_render["add_el"];
      $el.data('index', pred_list.indexOf(pred));
      $add_el.data('index', pred_list.indexOf(pred));
      pred.rendered = true;
      $add_el.click(function(e) {
        var idx;
        e.preventDefault();
        idx = $(this).data('index');
        pred = pred_list[idx];
        if (pred.location.tag == null) {
          $("#tag-add").data('pred', pred);
          $('#fname').val('');
          return $('#add-address').popup('open');
        } else {
          $("#tag-delete").data('pred', pred);
          return $('#delete-address').popup('open');
        }
      });
      $el.click(function(e) {
        var idx;
      ...
      $('#tag-add').unbind('click').bind('click').click(function(e) {
        e.preventDefault();
        pred = $(this).data('pred');
        if ($('#fname').val() != null) {
        ...
        parent.history.back();
        return $('#input-search input').trigger("keyup");
      });
      $('#tag-delete').unbind('click').bind('click').click(function(e) {
        e.preventDefault();
        pred = $(this).data('pred');
        pred.location.tag = null;
        pred.location.icon = null;
        location_favourites.remove(pred.location);
        parent.history.back();
        return $('#input-search input').trigger("keyup");
      });
      $li_el.append($el);
